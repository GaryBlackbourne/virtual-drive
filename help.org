...
This is how one mounts a file as a block device, and then unmountes it
#+begin_src bash

  sudo losetup -f image.img  # Mount file as loopdevice
  sudo losetup -d /dev/loop0 # Unmount loop device
  losetup -a                 # check mounted loopdevices
